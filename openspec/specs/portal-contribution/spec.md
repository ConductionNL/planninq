...
Planix's ADR-046 portal contribution: one plain, dependency-free provider class
(`OCA\Planix\Portal\PortalContributionProvider`) that declares, for the single
...
### Requirement: Planix ships the ADR-046 contractor portal contribution (REQ-PC-000)

The app MUST serve its entire portal contribution through the single plain,
dependency-free `OCA\Planix\Portal\PortalContributionProvider` class (duck-typed
...
planix, and no Nextcloud-uid identity field may appear in any read projection or
...
- @e2e exclude backend-only contract surface with no planix UI; the portal renders inside portaliq — covered by PHPUnit (tests/unit/Portal/PortalContributionProviderTest.php)

## Notes

- Discovery is pull-based from portaliq (`method_exists`, never `instanceof`);
  planix registers nothing in `lib/AppInfo/Application.php`.
