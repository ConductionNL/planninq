...
Four of planninq's five Playwright specs under `tests/e2e/` self-skip whenever
the environment lacks pre-existing planninq data, rather than creating that
data. Verified against `HEAD`:

- `tests/e2e/kanban-board.spec.ts:20-22` (docblock): *"The board surface
  depends on planninq being installed with at least one project the admin is a
...
- `tests/e2e/task-collaboration.spec.ts:25-27`: *"The task detail surface and
  the Activity entries depend on planninq being installed with seeded data;
  these tests skip cleanly..."*
- `tests/e2e/label-management.spec.ts:20-22`: *"Planninq is not installed in
...
planninq — so on every CI run today these specs hit their `test.skip()` guards
...
- Add a fixture-seeding step to `tests/e2e/global-setup.ts` (or a new
  ...
  planninq/OpenRegister REST API directly (matching the pattern
  `tests/integration/planninq.postman_collection.json` already uses for
