...
- [x] 2.1 `tests/e2e/kanban-board.spec.ts`: kept the "Planninq not installed" skip; converted the board/cards guards to `await expect(board).toHaveCount(1)` / `await expect(cards).not.toHaveCount(0)`, and the badge counts to `await expect(dueSoon.first()).toBeVisible()` / `overdue` visible
