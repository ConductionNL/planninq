# Task Collaboration Specification (Delta)

**Status**: proposed
**Scope**: planix
**OpenSpec changes**:
- [task-collaboration-sidebar](../../) — adds comments, file attachments, and activity/audit trail on the task detail sidebar

## Purpose

Defines the collaboration surface of a task: comments via Nextcloud's `ICommentsManager` (through OpenRegister's notes API), file attachments via Nextcloud Files (through OpenRegister's object-files API), the OpenRegister audit trail, and publication of task events to the Nextcloud Activity stream. All three sidebar tabs are presentation over existing NC/OR abstractions — planix MUST NOT introduce an app-local comment or attachment schema, and MUST NOT add pass-through controllers for them (ADR-022).
...
- **Architecture:** zero planix pass-through controllers for comments/files/audit trail (ADR-022, gate-17); the only new PHP is the Activity provider/filter/listener
- **Internationalization:** all tab labels, empty states, and activity subjects ship en + nl; English source strings are the keys (ADR-007)
- **Performance:** sidebar tabs lazy-load per tab activation; Activity publishing is event-driven (no polling, no cron)
- **Accessibility:** tabs and the comment composer meet WCAG AA using standard NC components

## Acceptance Criteria

- [ ] Task detail sidebar shows Comments, Files, and Audit Trail tabs (tags/tasks generic tabs hidden)
- [ ] Comments: add/list for task readers; edit/delete own only; ICommentsManager-backed via OR notes; no app-local schema
- [ ] Attachments: upload/list/download/remove; file exists in NC Files via the OR object folder
- [ ] Audit trail: read-only who/when/what with old → new values from OR
- [ ] Activity: created/status/assignee/dueDate/deleted entries reach project members (not the actor), en + nl, behind a "Planix" filter
- [ ] No Nextcloud notifications are created by the activity pipeline
- [ ] gate-17 (redundant-controller) and gate-18 (notification-dialect) pass unchanged

## Notes

- Verified present in OpenRegister (2026-06-11): notes routes (`notes#index/create/update/destroy` in `appinfo/routes.php`), `NoteService` wrapping `ICommentsManager` (objectType `openregister`), object files, audit trails; `@conduction/nextcloud-vue` ships the `notes`/`files`/`auditTrail` sidebar tabs and `filesPlugin`/`auditTrailsPlugin` store plugins.
- `@mention` in comments and the `task_commented` notification (with the existing `notify_commented` user toggle) are V1; the notification dispatch must then follow ADR-031, not an app-local sender.
- The activity subject for assignment uses a distinct key (`task_assigned_activity`) so it cannot collide with the `task_assigned` notification subject in `tasks.md`.
