...
- [x] `appinfo/info.xml`: bump `<version>` per the immutable-asset cache-bust
  convention (patch bump from the current value). **[config]**

## 4. Verify

- [x] Confirm no other file needs editing: `LICENSE`, `composer.json`,
  `publiccode.yml`, `README.md`, and PHP `@license` docblocks already declare
  EUPL-1.2 — assert they are unchanged. **[config]**
- [x] `xmllint --noout appinfo/info.xml` (or `composer run lint` if wired) to
  confirm the manifest still validates against `info.xsd`. **[config]**
- [x] Grep the tree for any remaining `agpl`/`AGPL` occurrence that refers to
