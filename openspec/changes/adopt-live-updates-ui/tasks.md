...
- [x] 2. Create `src/store/objectStore.js` exporting a Planix-owned `createObjectStore('planix-objects')` instance; swap the four shared-store import sites (`store/projects.js`, `store/timeEntries.js`, `store/store.js`, `views/Timesheet.vue`) to it.
