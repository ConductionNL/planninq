# Tasks: Planninq Beta Cross-Surface Alignment
...
- [x] 2.1 Author `conduction-website/src/pages/apps/planninq.mdx` from the `shillinq.mdx` template: DetailHero (Beta, v0.2, cobalt), 4-item FeatureList, PairRow (OpenRegister, Procest), PartnersForApp, CtaBanner
- [x] 2.2 Author the Dutch mirror `conduction-website/i18n/nl/docusaurus-plugin-content-pages/apps/planninq.mdx`
- [x] 2.3 Add the missing `planninq` row to `conduction-website/src/data/apps-catalog.js` PRESENTATION (tagline, href, Processes category, AppGlyph) so the page surfaces on `/apps`
- [x] 2.4 Add an explicit `planninq: 'PX'` entry to `AppGlyph.jsx` MONOGRAMS

## 3. Docs (`planninq/docs/`)
