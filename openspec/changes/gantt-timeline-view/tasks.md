...
- [ ] 1.2 Register `timeline#forProject` (GET `/api/projects/{projectId}/timeline`) in `appinfo/routes.php` with explicit auth; resolves to `TimelineController::forProject` (route-auth + route-reachability PASS).
  - **spec_ref**: `specs/gantt-timeline-view/spec.md#requirement-a-projects-tasks-can-be-viewed-on-a-time-axis`
  - **acceptance_criteria**:
...
- [ ] 3.1 `openspec validate gantt-timeline-view --strict` clean; PHPUnit for the controller green; vitest for the view; no dangling refs; route resolves; the view creates/mutates no objects (read-only invariant).
  - **spec_ref**: all
  - **acceptance_criteria**:
