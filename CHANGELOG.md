...
- **Renamed the app from Planix to Planninq.** The Nextcloud app id is now `planninq`, the PHP namespace is `OCA\Planninq`, every `/apps/planix/…` URL is now `/apps/planninq/…`, the l10n domain is `planninq`, and `lib/Settings/planix_register.json` is now `lib/Settings/planninq_register.json`. Because Nextcloud has no in-place app-id upgrade — a new `<id>` is simply a different app — a `MigrateAppConfigKeys` repair step copies every stored `IAppConfig` value from the `planix` namespace to `planninq`; it is registered under **both** `<install>` and `<post-migration>` and runs **before** `InitializeSettings` (see the ordering note in `appinfo/info.xml`). It is copy-only and idempotent, so the old rows survive a rollback.
  - **Not migrated by this release:** pre-rename `oc_activity` rows (app `planix`, type `planix_task`) are not rewritten, so activity history published before the rename is no longer shown by the app's Activity filter.
- **Renamed the OpenRegister register slug from `planix` to `planninq`.** The register slug is a separate identifier from the app id and was deliberately left behind by the app-id rename above; this change moves it, together with every `register` / `registerSlug` / `objects/<slug>` reference in PHP, Vue, the AppHost manifest, the Postman collection and the e2e seeders. A new `MigrateRegisterSlug` repair step renames the register **row** first, registered under **both** `<install>` and `<post-migration>`, ahead of `InitializeSettings`. This is required, not cosmetic: OpenRegister's `ImportHandler` resolves a register by slug and by nothing else, and its `DoesNotExistException` branch is the *create a new one* path — so a renamed slug in the register JSON without the row rename would silently fork a second, empty register and leave every stored object behind on the old row, reachable by nothing and with no error anywhere. The step is idempotent, non-destructive, never throws (an escaping exception under `<install>` aborts the install), refuses rather than merges when both slugs already exist, and also re-points a stored app-config `register` value that still names the old slug — guarded on the value, so a numeric register id is left alone.
  - **It moves no data.** An object is bound to its register by numeric id: the `_register` column holds the id and the shard tables are named `oc_openregister_table_<registerId>_<schemaId>`. The slug appears nowhere in the physical layout, so this is a one-column `UPDATE` on one row and every object, table, schema link and folder follows it untouched.
  - **Still `planix` on purpose:** the register's `tablePrefix` and `folder`, which name physical things that already exist and would be stranded rather than renamed; the `planix.conduction.nl` docs host, which a separate change handles; and `MigrateAppConfigKeys::OLD_APP_ID` / `MigrateUserPreferences::OLD_APP_ID`, which are the rename machinery naming the source namespace they read from.
