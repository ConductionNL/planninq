...
This page gets a working Planninq running on your own machine in two commands. You end with planning and scheduling over registry objects.
...
```bash
curl -fsSLO https://raw.githubusercontent.com/ConductionNL/planninq/development/planninq-compose.yaml
```
...
==> installing planninq <version>
==> apps present: integriq openregister planninq thematiq
```
...
```bash
curl -s http://localhost:8620/status.php
```
...
| What | Where |
| --- | --- |
| **Planninq** | [http://localhost:8620/apps/planninq/](http://localhost:8620/apps/planninq/) |
| Admin interface | [http://localhost:8620](http://localhost:8620) — `admin` / `admin` |
...
| `planninq` | The app this page is about. |

That OpenRegister dependency is **not declared** in `appinfo/info.xml` — no app in the fleet declares an `<app>` dependency — so nothing stops the App Store from installing planninq without it. It would then load, find no register to attach to, and show you an empty app rather than an error. The compose file encodes the dependency the manifest does not.
...
curl -s -o /dev/null -w '%{http_code}\n' -u admin:admin -L "http://localhost:8620/apps/planninq/"
...
curl -s -u admin:admin "http://localhost:8620/apps/openregister/api/registers" | head -c 300
```
...
```bash
# Stop, keep the data
docker compose -f planninq-compose.yaml down

# Stop and delete everything, including the database
docker compose -f planninq-compose.yaml down -v
```
...
**Everything returns 404 or a maintenance page after a restart.** Nextcloud is waiting for an upgrade. Run `docker compose -f planninq-compose.yaml exec -u www-data nextcloud php occ upgrade`.
