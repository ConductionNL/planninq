...
async function dismissFirstRunWizard(
  ...
  try {
    const res = await ctx.delete("/index.php/apps/firstrunwizard/wizard", {
      failOnStatusCode: false,
    });
    // eslint-disable-next-line no-console
    console.log(
      `[playwright globalSetup] first-run wizard dismissal returned ${res.status()}`,
    );
  } catch (err) {
    // eslint-disable-next-line no-console
    console.warn(
      `[playwright globalSetup] could not dismiss the first-run wizard: ${(err as Error).message}`,
    );
  } finally {
    await ctx.dispose();
  }
...
async function pinUserLanguageToEnglish(
  ...
  try {
    ...
    if (!res.ok()) {
      // eslint-disable-next-line no-console
      console.log(
        `[setup] could not pin language to en (status ${res.status()}) — ` +
    ...
    // eslint-disable-next-line no-console
    console.log(
      `[setup] could not pin language to en: ${(err as Error).message}`,
    );
  } finally {
    await ctx.dispose();
  }
...
export default async function globalSetup(config: FullConfig): Promise<void> {
  ...
  const username =
    process.env.NC_ADMIN_USER ?? process.env.ADMIN_USER ?? "admin";
  const password =
    process.env.NC_ADMIN_PASS ?? process.env.ADMIN_PASSWORD ?? "admin";
  ...
  try {
    ...
    // eslint-disable-next-line no-console
    console.warn(
      `[playwright globalSetup] fixture seeding failed: ${(err as Error).message}`,
    );
  }
