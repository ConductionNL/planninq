 ...
 *   PLANNINQ_E2E_ALLOW_SHARED_INSTANCE  this app only
 *
...
export const APP_ID = 'planninq'

/** The app-specific opt-in variable. Holds an origin, not a boolean. */
export const SHARED_INSTANCE_FLAG = 'PLANNINQ_E2E_ALLOW_SHARED_INSTANCE'
...
export function permittingFlag(
	...
	for (const name of SHARED_INSTANCE_FLAGS) {
		...
		if (normaliseOrigin(value) === wanted) {
			return { name, value }
		}
	}
	return null
...
export function assertInstancePermitted(
	...
	if (isSharedOrigin(target) === false) {
		return target
	}
	if (permittingFlag(target, env) !== null) {
		return target
	}
...
export function refuseOnSharedInstance(
	...
	if (isSharedInstance(target) === false) {
		return
	}
	throw new Error(`[${APP_ID} e2e] ${what} must not run on ${target}.\n`
		+ `${reason}\n`
 ...
 *
 * Resolution order:
 *  1. `PLANNINQ_E2E_OCC`, a complete prefix, for any rig shape the guesses
 *     below do not cover.
 *  2. `PLANNINQ_E2E_CONTAINER` or `NEXTCLOUD_CONTAINER`, a container name,
...
export function occPrefix(env: NodeJS.ProcessEnv = process.env): string[] {
	const explicit = (env.PLANNINQ_E2E_OCC ?? '').trim()
	if (explicit !== '') {
		return explicit.split(/\s+/).filter((p) => p !== '')
	}

	const container = (
		env.PLANNINQ_E2E_CONTAINER
