...
describe(`${APP_ID} shared-instance guard`, () => {
	...
	it('refuses a shared instance that no flag names', () => {
		expect(() => assertInstancePermitted('http://localhost:8080', NONE)).toThrow(/SHARED development instance/)
	})
	...
	it('permits the origin the flag names, in any loopback spelling', () => {
		...
		expect(assertInstancePermitted('http://localhost:8080', env)).toBe('http://localhost:8080')
	})
	...
	it('lets a disposable rig through without a flag', () => {
		expect(assertInstancePermitted('http://localhost:8095', NONE)).toBe('http://localhost:8095')
	})
	...
	it('binds occ to a named container, and falls back to the server root', () => {
		...
		expect(occPrefix(env).join(' ')).toBe('docker exec -u www-data nextcloud php occ')
	})
})
